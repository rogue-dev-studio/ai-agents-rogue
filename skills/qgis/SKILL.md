...
QGIS GIS operations MCP server. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `qgis` or user asks by name
...
2. **Confirm tooling** - Verify `qgis` runtime/CLI/MCP is available; document install if missing.
