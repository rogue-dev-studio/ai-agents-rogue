...
**Level: max** specialized skill (not merged). Category hint: `base`.

## Summary

Global mise configuration and settings. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `mise-config` or user asks by name
...
2. **Confirm tooling** - Verify `mise-config` runtime/CLI/MCP is available; document install if missing.
