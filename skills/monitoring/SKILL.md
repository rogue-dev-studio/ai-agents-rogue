...
**Level: max** specialized skill (not merged). Category hint: `monitoring`.

## Summary

Claude monitoring and usage tracking tools (UV, claude-monitor, claude-usage-cli). Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `monitoring` or user asks by name
...
2. **Confirm tooling** - Verify `monitoring` runtime/CLI/MCP is available; document install if missing.
