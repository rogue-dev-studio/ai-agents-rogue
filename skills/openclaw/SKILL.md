...
**Level: max** specialized skill (not merged). Category hint: `ai-dev`.

## Summary

OpenClaw - Multi-channel AI gateway for messaging platforms with browser Control UI. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `openclaw` or user asks by name
...
2. **Confirm tooling** - Verify `openclaw` runtime/CLI/MCP is available; document install if missing.
