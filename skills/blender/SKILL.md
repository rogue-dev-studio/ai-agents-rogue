...
name: blender
description: >-
  Blender 3D modeling MCP server. Use when the project needs this capability or the user / team manifest asks for it. Use for specialized `blender` work when listed in TEAM.yaml or explicitly requested.
experience_level: max
---

# blender

**Level: max** specialized skill (not merged). Category hint: `desktop`.

## Summary

Blender 3D modeling MCP server. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `blender` or user asks by name
...
2. **Confirm tooling** - Verify `blender` runtime/CLI/MCP is available; document install if missing.
