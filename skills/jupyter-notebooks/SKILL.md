...
**Level: max** specialized skill (not merged). Category hint: `dev-tools`.

## Summary

Jupyter notebook execution MCP server. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `jupyter-notebooks` or user asks by name
...
2. **Confirm tooling** - Verify `jupyter-notebooks` runtime/CLI/MCP is available; document install if missing.
