...
Instance agent for mimir fleet management. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `draupnir` or user asks by name
...
2. **Confirm tooling** - Verify `draupnir` runtime/CLI/MCP is available; document install if missing.
