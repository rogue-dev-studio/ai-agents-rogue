...
PBR material generation MCP server (GPU required). Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `pbr-rendering` or user asks by name
...
2. **Confirm tooling** - Verify `pbr-rendering` runtime/CLI/MCP is available; document install if missing.
