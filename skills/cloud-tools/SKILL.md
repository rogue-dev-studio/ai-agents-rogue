...
AWS, Azure, GCP CLIs. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `cloud-tools` or user asks by name
...
2. **Confirm tooling** - Verify `cloud-tools` runtime/CLI/MCP is available; document install if missing.
