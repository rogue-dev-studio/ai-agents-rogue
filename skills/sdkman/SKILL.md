...
**Level: max** specialized skill (not merged). Category hint: `package-manager`.

## Summary

SDKMAN - The Software Development Kit Manager for JVM tools. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `sdkman` or user asks by name
...
2. **Confirm tooling** - Verify `sdkman` runtime/CLI/MCP is available; document install if missing.
