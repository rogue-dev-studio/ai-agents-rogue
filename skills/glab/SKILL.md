...
**Level: max** specialized skill (not merged). Category hint: `devops`.

## Summary

GitLab CLI for merge requests, issues, pipelines, and CI/CD. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `glab` or user asks by name
...
2. **Confirm tooling** - Verify `glab` runtime/CLI/MCP is available; document install if missing.
