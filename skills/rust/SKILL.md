...
name: rust
description: >-
  Rust development environment. Use when the project needs this capability or the user / team manifest asks for it. Use for specialized `rust` work when listed in TEAM.yaml or explicitly requested.
...
Rust development environment. Use when the project needs this capability or the user / team manifest asks for it.

## When to use

- `TEAM.yaml` includes `rust` or user asks by name
...
2. **Confirm tooling** - Verify `rust` runtime/CLI/MCP is available; document install if missing.
